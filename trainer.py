...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, SegHeatmapConsLoss, \
    HybridWeightedLoss
...
class nnUNetTrainer(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            # self.loss = self._build_loss(lambda_imf=1.0) #####9.26
    ...
    def _build_loss(self):
        ...
        if self.label_manager.has_regions:
            ...
            loss = DC_and_CE_loss(
                ...
                ce_kwargs={
                    'weight': torch.tensor([1.0, 1.5, 1.0, 1.0, 1.0]).to(self.device)
                },
                weight_ce=1,
                weight_dice=1,
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        # 新增写入refiner分支class1 Dice
        # self.logger.log('refiner_class1_dice', refiner_dice_class1, self.current_epoch)
    ...
    def on_epoch_start(self):
        ...
        elif hasattr(self.loss, 'loss') and hasattr(self.loss.loss, 'set_epoch'):
    ...
    def on_epoch_end(self):
        ...
        self.print_to_log_file('Dice per class/region', [np.round(i, 4) for i in dice_per_class])
        ...
        self.print_to_log_file(f"Epoch time: {np.round(epoch_time, 2)} s")

        # ------------------------------
        # 保存最新 checkpoint（每 save_every 个 epoch）
        # ------------------------------
        ...
        # ------------------------------
        # EMA pseudo Dice（训练平滑参考/early stopping）
        # ------------------------------
        ema_dice = self.logger.my_fantastic_logging.get('ema_fg_dice', [None])[-1]
        if ema_dice is None:
            self.print_to_log_file("⚠️ EMA Dice is None this epoch, skipping EMA comparison.")
        else:
            # 初始化 _best_ema 与计数器
            if not hasattr(self, '_best_ema') or self._best_ema is None or not isinstance(self._best_ema, (float, int)):
                self._best_ema = ema_dice
                self._ema_patience_counter = 0
            else:
                # 比较并更新
                if ema_dice > self._best_ema:
                    self._best_ema = ema_dice
                    self._ema_patience_counter = 0
                    self.save_checkpoint(join(self.output_folder, 'checkpoint_best_ema.pth'))
                    self.print_to_log_file(f"Yayy! New best EMA pseudo Dice: {np.round(self._best_ema, 4)}")
                else:
                    self._ema_patience_counter += 1

            # Early stopping based on EMA
            patience_limit = 20  # 连续多少个 epoch 没提升就停止训练
            if self._ema_patience_counter >= patience_limit:
        ...
        if not hasattr(self, '_best_val_dice'):
            self._best_val_dice = None
        if self._best_val_dice is None or val_dice > self._best_val_dice:
            self._best_val_dice = val_dice
            self.save_checkpoint(join(self.output_folder, 'checkpoint_best_val.pth'))
            self.print_to_log_file(f"Yayy! New best Val Dice: {np.round(self._best_val_dice, 4)}")

        # ------------------------------
        # 小目标 class1 Dice（单独关注）
        # ------------------------------
        class1_dice = dice_per_class[0]  # index 0 为类别1
        if not hasattr(self, '_best_class1_dice'):
            self._best_class1_dice = None
        if self._best_class1_dice is None or class1_dice > self._best_class1_dice:
            self._best_class1_dice = class1_dice
            self.save_checkpoint(join(self.output_folder, 'checkpoint_best_class1.pth'))
            self.print_to_log_file(f"Wow! New best Class 1 Dice: {np.round(self._best_class1_dice, 4)}")
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            self.current_epoch = epoch  # 确保 current_epoch 被更新

            # ✅ 告诉 loss 当前是第几轮，用于 warmup
            if hasattr(self.loss, 'set_epoch'):
